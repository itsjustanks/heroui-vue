...
export default defineComponent(() => {
  const categories = [{
    ...
    items: [{
      content: "You can reach our support team through the contact form on our website, or email us directly at support@example.com.",
      title: "How do I get support?"
    }],
    title: "Support"
  }];
  return () => <div class="flex w-full flex-col gap-6">
      ...
      {categories.map(category => <div key={category.title}>
          ...
          <Accordion class="w-full" variant="surface">
            {category.items.map((item, index) => <Accordion.Item key={index}>
                <Accordion.Heading>
                  <Accordion.Trigger>
                    {item.title}
                    <Accordion.Indicator>
