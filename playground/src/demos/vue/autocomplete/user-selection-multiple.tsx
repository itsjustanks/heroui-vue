...
import { Autocomplete, Avatar, AvatarFallback, AvatarImage, Description, EmptyState, Label, ListBox, SearchField, Tag, TagGroup, useFilter } from "@itsjustanks/heroui-vue";
...
export default defineComponent(() => {
  ...
  return () => <Autocomplete class="w-[256px]" defaultValue={["1", "2"]} placeholder="Select your teammates" selectionMode="multiple" value={selectedKeys.value} onChange={keys => selectedKeys.value = keys as Key[]}>
      <Label>Users</Label>
      <Autocomplete.Trigger>
        <Autocomplete.Value>
          ...
          return <TagGroup size="sm" onRemove={onRemoveTags}>
                ...
                return <Tag key={selectedItem.id} id={selectedItem.id}>
      ...
      <Autocomplete.Popover>
        <Autocomplete.Filter filter={contains}>
          ...
          <ListBox renderEmptyState={() => <EmptyState>No results found</EmptyState>}>
            {users.map(user => <ListBox.Item key={user.id} id={user.id} textValue={user.name}>
                <Avatar size="sm">
