import type { SortDescriptor } from "@itsjustanks/heroui-vue";
import type { SortingState } from "@tanstack/react-table";
import { Chip, Pagination, Table, cn } from "@itsjustanks/heroui-vue";
...
const columns = [columnHelper.accessor("name", {
  ...
  cell: info => <Chip color={statusColorMap[info.getValue()]} size="sm" variant="soft">
        {info.getValue()}
      </Chip>,
  header: "Status"
...
export default defineComponent(() => {
  ...
  const sortDescriptor = computed(() => toSortDescriptor(sorting.value));
  const {
    pageIndex
  ...
  return () => <Table>
      <Table.ScrollContainer>
        <Table.Content aria-label="TanStack Table example" class="min-w-[600px]" sortDescriptor={sortDescriptor.value} onSortChange={d => sorting.value = toSortingState(d)}>
          <Table.Header>
            {table.getHeaderGroups()[0]!.headers.map(header => <Table.Column key={header.id} allowsSorting={header.column.getCanSort()} id={header.id} isRowHeader={header.id === "name"}>
                {({
              sortDirection
          ...
          <Table.Body>
            {table.getRowModel().rows.map(row => <Table.Row key={row.id} id={row.id}>
        ...
        <Pagination size="sm">
          ...
          <Pagination.Content>
            ...
            {pages.map(p => <Pagination.Item key={p}>
                <Pagination.Link isActive={p === pageIndex + 1} onPress={() => table.setPageIndex(p - 1)}>
                  {p}
                </Pagination.Link>
              </Pagination.Item>)}
            <Pagination.Item>
