...
if (ids.length === 0) {
  ids = readdirSync(join(HERE, 'src/demos/vue'))
...
async function shoot (id) {
  ...
  try {
    await page.goto(`${BASE}/?c=${id}`, { waitUntil: 'domcontentloaded', timeout: 12000 })
    await page.waitForTimeout(900)
    const opts = { path: `/tmp/shot-${id}.png`, animations: 'disabled', timeout: 7000 }
